...
const ClientApp = {
 ...
 // Auto-refresh KDS every 5 seconds
 this.pollInterval = setInterval(() => this.fetchKDSOrders(), 5000);
 },
 ...
 if (data.success) {
 // Play audio chime if new order arrived
 if (this.soundEnabled && data.orders.length > this.prevOrderCount && this.prevOrderCount > 0) {
 SatayApp.playChime('new_order');
 SatayApp.showToast(' New order received on grill queue!', 'warning');
 }

 this.prevOrderCount = data.orders.length;
 this.activeOrders = data.orders;

 this.renderGrillAggregator(data.grill_summary);
 this.renderKDSBoard();
  ...
  renderGrillAggregator(summary) {
    ...
    bar.innerHTML = `
      <div style="font-weight:800; font-size:1.05rem; display:flex; align-items:center; gap:6px; color:var(--primary);">
        <span>🔥 Grill Queue:</span> <strong>${summary.total_sticks || 0} Sticks</strong>
      </div>
      <div style="display:flex; gap:0.5rem; flex-wrap:wrap; align-items:center;">
        ${summary.tulang_madu > 0 ? `<div class="meat-chip">🍯 Tulang Madu: <strong style="color:var(--gold);">${summary.tulang_madu}</strong></div>` : ''}
        ${summary.ayam > 0 ? `<div class="meat-chip">🍗 Ayam: <strong style="color:var(--gold);">${summary.ayam}</strong></div>` : ''}
        ${summary.daging > 0 ? `<div class="meat-chip">🥩 Daging: <strong style="color:var(--gold);">${summary.daging}</strong></div>` : ''}
        ${summary.other > 0 ? `<div class="meat-chip">Other: <strong style="color:var(--primary);">${summary.other}</strong></div>` : ''}
        ${summary.total_sticks === 0 ? `<span style="font-size:0.85rem; color:var(--text-muted);">Grill is clear. Ready for orders!</span>` : ''}
      </div>
    `;
  },
 ...
 <p>All incoming orders will appear here with live stick counters and timers.</p>
 </div>
 `;
 return;
 }
 ...
 <span>Mode: <strong style="color:#fff; text-transform:capitalize;">${order.dining_type.replace('_', ' ')}</strong></span>
 <span>Pay: <strong style="color:${order.payment_status === 'paid' ? 'var(--success)' : 'var(--gold)'};">${order.payment_status}</strong></span>
 </div>
 ...
 return `
 <button class="btn btn-secondary btn-sm" style="flex:1;" onclick="SatayApp.printThermalReceipt(${JSON.stringify(order).replace(/"/g, '&quot;')})"> Receipt</button>
 ...
 <div>
 <div style="font-weight:700; font-size:1rem; margin-bottom:2px;">${item.name}</div>
 <div style="color:var(--gold); font-weight:800; font-size:1.1rem; margin-bottom:0.75rem;">
 ${SatayApp.formatPrice(item.price_per_unit)} <span style="font-size:0.75rem; color:var(--text-muted);">/ ${item.unit_name}</span>
 </div>
 </div>
          ...
          <td>
            <div style="display:flex; align-items:center; gap:10px;">
              ...
              <div>
                ...
                <div style="font-size:0.75rem; color:var(--text-muted);">${item.unit_name ? `per ${item.unit_name}` : ''} ${item.min_quantity > 1 ? `(Min ${item.min_quantity})` : ''}</div>
              </div>
            </div>
          </td>
